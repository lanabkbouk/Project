...
// needed volunteers, register window, age range, skills)
...
export const opportunitySchema = z
  .object({
    ...
    // للمتطوعين (سنا: التصنيف يعتمد على المهارات والعمر والمدينة)
    ...
    // نطاق العمر اختياري — فراغ يعني بلا قيد من هالجهة
    minAge: z
      .union([z.coerce.number().int().min(14, 'Minimum age must be at least 14.'), z.literal('')])
      .optional(),
    maxAge: z
      .union([z.coerce.number().int().max(100, 'Maximum age must not exceed 100.'), z.literal('')])
      .optional(),
  })
  ...
  .refine(
    (data) => data.minAge === '' || data.maxAge === '' || Number(data.maxAge) >= Number(data.minAge),
    {
